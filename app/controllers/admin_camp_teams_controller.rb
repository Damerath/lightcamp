class AdminCampTeamsController < ApplicationController
  ...
  def week_plan_print
    ...
    return redirect_to admin_camp_team_page_path(@camp, @camp_team), alert: "Kein veröffentlichter Wochenplan vorhanden." if @week_plan_source_team.blank?

    prepare_week_plan_print
    render "camp_teams/week_plan_print"
  end

  def sport_tournament_print
    return redirect_to admin_camp_team_page_path(@camp, @camp_team, section: "sport_plan"), alert: "Nur fuer das Sport-Team verfuegbar." unless @camp_team.sport_team?

    @camp_sport_tournament_plan = @camp_team.ensure_sport_tournament_plan!
    render "camp_teams/sport_tournament_print"
  end

  def kitchen_plan_print
    return redirect_to admin_camp_team_page_path(@camp, @camp_team, section: "kitchen_plan"), alert: "Nur fuer das Küchen-Team verfuegbar." unless @camp_team.kitchen_team?

    @camp_team.sync_kitchen_day_plans_to_schedule!
    @camp_kitchen_day_plans = @camp_team.camp_kitchen_day_plans.ordered
    render "camp_teams/kitchen_plan_print"
  end

  def diy_plan_print
    return redirect_to admin_camp_team_page_path(@camp, @camp_team, section: "diy_plan"), alert: "Nur fuer das DIY-Team verfuegbar." unless @camp_team.diy_team?

    @camp_team.sync_diy_day_plans_to_schedule!
    @camp_diy_day_plans = @camp_team.camp_diy_day_plans.ordered
    render "camp_teams/diy_plan_print"
  end

  def room_plan_print
    return redirect_to admin_camp_team_page_path(@camp, @camp_team, section: "room_plan"), alert: "Nur fuer das Freizeitleiter-Team verfuegbar." unless @camp_team.camp_leader_team?
  ...
  def workspace_page_path(section: nil)
    admin_camp_team_page_path(@camp, @camp_team, section: section)
  end

  def camp_team_params
    permitted = params.require(:camp_team).permit(:name, :capacity, :male_slots, :female_slots, :responsible_slots, :position, :next_internal_meeting_on, :next_internal_meeting_hour, :next_internal_meeting_minute, :week_plan_published, :custom_description, :custom_responsible_description)
    attributes = permitted.slice(:name, :capacity, :male_slots, :female_slots, :responsible_slots, :position)
    ...
    attributes[:custom_description] = permitted[:custom_description] if params[:camp_team].key?(:custom_description)
    attributes[:custom_responsible_description] = permitted[:custom_responsible_description] if params[:camp_team].key?(:custom_responsible_description)
  ...
  def update_return_path
    ...
    if return_to.start_with?("/") && !return_to.start_with?("//")
      return_to
    elsif params[:from] == "team_page"
      admin_camp_team_page_path(@camp, @camp_team, section: params[:section].presence || "overview")
    else
      admin_camp_teams_path(@camp)
    end
  end

  def update_failure_path
    if params[:from] == "team_page"
      admin_camp_team_page_path(@camp, @camp_team, section: params[:section].presence || "overview")
    else
      admin_camp_teams_path(@camp, modal: "edit_team", camp_team_id: @camp_team.id)
    end
  end
