class CampApplication < ApplicationRecord
  ...
  validate :sleeping_place_matches_assigned_camp
  ...
  def sleeping_place_matches_assigned_camp
    return if camp_sleeping_place.blank?
    return if assigned_camp.present? && camp_sleeping_place.camp_id == assigned_camp.id

    errors.add(:camp_sleeping_place, "muss zu dem zugewiesenen Camp gehören")
  end
end
